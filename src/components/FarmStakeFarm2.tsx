...
import {FarmContractTYpe, getLockTimeFrameQueryFarm2, useFindUsersStakedTimeFarm2} from "../data/farming/FarmV2";
...
interface Props {
  farmResponseFun: (res: DeliverTxResponse | undefined, errors: PostError | undefined, type?: string) => void,
...
const FarmStakeFarm2 = ({type, dataSource, farmResponseFun, pageName, farmContractType, hidden = true}: Props) => {
  ...
  const collapsedIcon = <img src={expand ? collapsed : plus_icon} alt={""} className={styles.expand_icon} onClick={()=> setExpand(!expand)} />
    ...
    <div>
      <Card className={classNames(styles.main, expand ? '' : styles.slim)} >
        <Grid className={styles.stake_container} onClick={()=> setExpand(!expand)}>
          <Grid className={classNames(styles.cell, styles.mobile_cell, styles.cellHead)}>
          ...
          <Grid  className={classNames(styles.cell, styles.collapsedIconContainer)} >
