...
function TradingChart(props) {
  ...
  // };
  const defaultProps = {
    symbol: "LOOP/USDC", // default symbol
    container: "tv_chart_container",
    datafeedUrl: "https://testbombay.loop.onl/v1/juno",
    libraryPath: "/charting_library/",
    chartsStorageUrl: "https://saveload.tradingview.com",
    chartsStorageApiVersion: "1.1",
    clientId: "tradingview.com",
  ...
  useEffect(() => {
    if (!pool || pool === "USDC") return
    if (!token1 || !token2) return
    let symbol, liquidity_pool, quote

    axios
      .get(
        `https://middlewareapi.loop.markets/v1/juno/pairDetail?pair1=${pool}`
      )
      .then((res) => {
        //quote: token1, base: token2
        res = res.data[pool]
        if (
          (token1 !== USDC_addr &&
            ((token1 && token1 === res.asset0.contractAddress) ||
              (token2 && token2 === res.asset1.contractAddress))) ||
          (token2 !== USDC_addr &&
            ((token2 && token2 === res.asset0.contractAddress) ||
              (token1 && token1 === res.asset1.contractAddress)))
        ) {
