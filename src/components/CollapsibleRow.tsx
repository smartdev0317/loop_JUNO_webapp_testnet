...
interface Props {
  ...
  pretax?: string
  hidden?: boolean
  farmResponseFun?: (res: DeliverTxResponse | undefined, error: PostError | undefined , type?: string) => void
  openModal: (
    record: Record,
    type: Type,
    token: string,
    ...
    <>
      ...
      >
        ...
        <td className={classNames(tdClassName)}>
          {currentUrl.includes("admin") ? '' :  <LinkButton className={styles.poolBtn} to={farmPage === FarmType.farm2 ? `${getPath(MenuKey.YEILD)}?tab=${MenuKey.POOL}` : `${getPath(MenuKey.YEILD)}?tab=${MenuKey.POOL_V2}`} >Pool</LinkButton> }
