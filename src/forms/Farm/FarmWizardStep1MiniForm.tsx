...
import {
  FarmContractTYpe, getLockTimeFrameQueryFarm2,
...
const FarmWizardStep1MiniForm = ({
  ...
  const { timeLeft: timeLeftv2, unStakeTimeLeft } = useUnstakeTimoutFarm2(findUserStakedTimeFarm2Fn?.(lpToken ?? ""), farmType, lpToken ?? ""
  )
  const { shortDayString, shortFormatTime} = unStakeTimeLeft

  const { messages: msgs,step: currentStep,  store } = useFarmMigrate()
