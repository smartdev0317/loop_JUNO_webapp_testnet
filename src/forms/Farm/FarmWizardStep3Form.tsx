...
const FarmWizardStep3Form = ({
  ...
  const token1Max = isNative(token1)
      ? lookupAmount(findBalanceFn(token1), 0)
      : lookupAmount(findBalanceFn(token1), token1Detail?.decimals)

  const token2Max = isNative(token2)
      ? lookupAmount(findBalanceFn(token2), 0)
      : lookupAmount(findBalanceFn(token2), token2Detail?.decimals)

  const leftt1 = minus(token1Max, div(amount, pow('10',token1Detail?.decimals ?? 6 )))
  const leftt2 = minus(token2Max,estimated)

  const dust1 = decimal(plus(leftt1, fpool1),6)
  const dust2 = decimal(plus(leftt2, fpool2), token2Detail?.decimals ?? 6)

  const bal1txt = gt(dust1, "0") ? `${commas(dust1)} ${lookupSymbol(token1Detail?.tokenSymbol)}` : ""
  const bal2txt = gt(dust2, "0") ? `${commas(dust2)} ${lookupSymbol(token2Detail?.tokenSymbol)}` : ""
 ...
 /* const [response, setResponse] = useState<DeliverTxResponse | undefined>(undefined)
