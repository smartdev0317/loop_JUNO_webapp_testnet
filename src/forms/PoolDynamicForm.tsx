...
import {LP, MIN_FEE, SMALLEST, USDC, UST} from "../constants"
import {div, gt, lt, max, minus, multiple, number, plus, pow} from "../libs/math"
...
interface Prop {
    ...
    disableBtn?: boolean
    updateTabFn?: (tab: MenuKey) => void
    temp?: boolean
    setCurrentPair?: any
    setExpanded?: (type: any) => void
}
...
const PoolDynamicForm = ({
    ...
    const {values, setValue, setValues, getFields, attrs, invalid, reset, errors, touched} = form
    const {value, token1, token2, pair, lp} = values

    const estimatedErrors = errors['estimated'].length > 0 ? errors['estimated'] : ''
    ...
    const amount = lt(toAmount(value), '0') ? "0" : toAmount(value)
    ...
    const {data: poolResult, refetch} = useQuery<any>(
        '@token-list',
        async () => {
            if (client && address) {
                try {
                    const data: PairPool = await client.queryContractSmart(pair ?? "", {pool: {}})
                    return data
                } catch (e) {
    ...
    // const maxLiquidityT2 = findBalanceFn?.(token2) ?? "0"
    // const minLiquidityToken = gt(maxLiquidity, maxLiquidityT2) ? Key.token1 : Key.token2
    ...
    const config = {
        ...
        tokenIndex: 1,
        factoryType: version === 2 ? version : 0,
        disabled: version === 1 && window.location.hash === '#provide',
        modalTitle: type === Type.WITHDRAW ? 'Select a Pair' : undefined,
        skip: type === Type.PROVIDE ? ['juno1hnftys64ectjfynm6qjk9my8jd3f6l9dq9utcd3dy8ehwrsx9q4q7n9uxt'] : []
    }
    ...
    const lpAfterTx = {
        [Type.PROVIDE]: number(lpBeforeTx2).toString(),
    ...
    useEffect(() => {
        ...
        if (gt(div(toLP?.estimated ?? "0", SMALLEST), secondTokenMax)) {
            ...
            setValue(
                Key.value,
                isNative(token1) ? decimal(minus(calculatedVal, symbol1 ==='JUNO' ? '0.5' : '0'), 3) : calculatedVal
            )
        }
                ...
                max: {
                    [Type.PROVIDE]: gt(findBalanceFn(token1) ?? "0", "0")
                        ? () => {
                            ...
                            const deductTax = ['JUNO', 'UJUNO'].includes(symbol1.toUpperCase())
    ...
    const reveseCoins = ((isNative(token1) && isNative(token2)) && token2Detail?.tokenSymbol === 'USDC')

    const bothIBCFunds = ((isNative(token1) && isNative(token2)) && isIBC(token2)) ? [
        ...insertIf(isNative(token2), insertToken2Coins),
        ...insertIf(isNative(token1), insertToken1Coins),
    ] : [
        ...insertIf(isNative(token1), insertToken1Coins),
        ...insertIf(isNative(token2), insertToken2Coins)
    ]
...
//  const showError = fields["estimated"]?.help?.content >= fields[Key.value]?.help?.content ? false : fields[Key.value]?.help?.content >= fields["estimated"]?.help?.content ? false : true;
    ...
    const submitDisable = (hashName == '#provide' && version === 1) ?
        <Button onClick={() => updateTabFn(MenuKey.POOL_V2)} size={'lg'} type={'button'} className={styles.submitBtn}>Add
            Liquidity on Pool V2</Button> : null;

    const resetFunc = () => {
        history.push({
            pathname: getPath(MenuKey.YEILD)
        });
    }
    ...
    return (
        <CustomMsgFormContainer
            ...
            {...tax}
            headerBorder={true}
            showCheckbox={type === Type.PROVIDE}
            ...
            afterSubmitChilds={submitDisable}
        >
            {
                version === 2 ? <>
                        ...
                        <div className={styles.estimatedContainer}>
                            ...
                            <FormGroupV2 showPercentageButtonsOnTop={true} newPoolDesign={true}
                                         showPercentageButtons={showPercentageButton01 && showPercentageButton02}
                                         showBalance={true} {...fields[Key.value]} />
                                         <FormGroupError className={styles.feedbacks} {...fields[Key.value]} />
                                         </div>
                            <div className={styles.divider}/>
                            <div className={styles.estimatedContainer}>
                            <FormGroupV2 newPoolDesign={true} {...fields["estimated"]} />
                        {  (toLP?.estimated && gt(toLP?.estimated, '0') && estimatedErrors.length) ? <p className={styles.feedbacks}>{estimatedErrors}</p> : <></> }
                        </div>
                        </div>
                        <span>
