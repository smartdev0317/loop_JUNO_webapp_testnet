...
import { findUnstakeValues, findValue } from "./receiptHelpers"
...
export default (type: Type, gov: boolean,unstakeResponse?: boolean,vesteResponse?:DeliverTxResponse|undefined) => (logs: TxLog[]) => {
  const getSymbolFn = useFindTokenDetails()
  const val = unstakeResponse ? findUnstakeValues(logs) : findValue(logs)
  ...
  return [
    {
      ...
      content: formatAsset(
        amount,
        !gov ? `${lpSymbol ? lpSymbol : devSymbol ?? ""} ${vesteResponse ? 'LOOP' : 'LP'}` : LOOP
      ),
    },
  ]
}
