...
import { LOOP, SMALLEST, USDC, JUT } from "../../constants"
...
const Assets = ({
        ...
        : denom !== undefined
          ...
          : otherToken === undefined
            ? pair
              ...
              : ""
            : "",
      balance: noPair ? "0" : balance ? balance : "0",
      balanceSymbol: {
  ...
  useEffect(()=>{
    setQuick([USDC.toUpperCase(), ...insertIf(isPoolPage,'JUNO'), ...insertIf(isPoolPage,LOOP)].map((item)=>{
      return lists?.find(({tokenSymbol}) => tokenSymbol?.toUpperCase() === item.toUpperCase())
    }).filter((item)=> item))
    setQuickSecondList( [LOOP,...insertIf(isPoolPage,'JUNO'), USDC.toUpperCase()].map((item)=>{
      return lists?.find(({tokenSymbol}) => tokenSymbol?.toUpperCase() === item.toUpperCase())
    }).filter((item)=> item))
  },[lists])
            ...
            return (
              <li key={`${token}~${index}`}>
                <button
                  ...
                  onClick={() => noPair ? {} :onSelect(token, pair)}
                >
                  <Asset
                    {...item}
