...
import { aUST_TOKEN, COMMISSION, MAX_SPREAD, SMALLEST, USDC, USDC_addr, UUSD } from "../../constants"
...
import swapIcon from "../../../src/images/swaping.png"
...
import UstAustHighPriceImpactModal, { HighPriceImpactModal } from "../../components/Static/UstAustHighPriceImpaceModal"
...
import { tokensInfo } from "../../data/API/dashboard"
...
const ExchangeForm = ({
  ...
  const tokenInfo=useRecoilValue(tokensInfo)
  ...
  const validate = ({ value1, value2, token1, token2, pair }: Values<Key>) => {
    ...
    const symbol1 = token1Del ? lookupSymbol(token1Del.tokenSymbol) : ""
    return {
      [Key.value1]: v.amount(isNative(token1) ? value1 : multiple(value1, pow('10', token1Del?.decimals ?? "6") ?? SMALLEST), {
            symbol: '',
            min: "0.001",
            max: token1
                ? isNative(token1)
                    ? multiple(minus(max1, symbol1.toUpperCase() === 'JUNO' ? '0.1' : '0'), pow('10', token1Del?.decimals ?? SMALLEST))
                    : max1
                : undefined,
          },
          undefined, token1Del?.decimals?.toString() ?? SMALLEST.toString()),
  ...
  const initial = {
    [Key.value1]: "",
    [Key.value2]: "",
    [Key.token1]: "ibc/EAC38D55372F38F1AFD68DF7FE9EF762DCF69F26520643CF3F9D292A738D8034",
    [Key.token2]: "juno1qsrercqegvs4ye0yqg93knv73ye5dc3prqwd6jcdcuj8ggp6w0us66deup" ?? "",
    [Key.pair]: "juno1utkr0ep06rkxgsesq6uryug93daklyd6wneesmtvxjkz0xjlte9qdj2s8q" ?? "",
  }
  ...
  const { simulated: perSimulated, refetch: refetchPerSimulation, loading } = perSimulation
  ...
  }, [amount1, amount2, value1, value2, time])
  ...
  useEffect(() => {
    ...
    setValues((values) => ({
      ...values,
      [key]: decimal(value, reverse ? token1Detail?.decimals ?? 6 : token2Detail?.decimals ?? 6),
    }))
  ...
  const config1: Config = {
    ...
    tokenIndex: 1,
    modalTitle:'From'
    // skip: ['juno14lycavan8gvpjn97aapzvwmsj8kyrvf644p05r0hu79namyj3ens87650k'],
          ...
          gt(findBalanceFn(token1) ?? "", "0")
              ? () => {
                const bal = token1
                    ...
                    : ""
                setValue(Key.value1, symbol1.toUpperCase() === 'JUNO' ? decimal(minus(gte(bal, "0.1") ? bal : "0", gte(bal, "0.1") ? "0.1" : "0"), token1Detail?.decimals ?? 6) : decimal(bal, token1Detail?.decimals ?? 6))
              }
              : undefined,
      maxValue:
          gt(findBalanceFn(token1) ?? "", "0")
              ? () => {
                ...
                return Key.value1, symbol1.toUpperCase() === 'JUNO' ? decimal(bal ?? "0", token1Detail?.decimals ?? 6) : decimal(bal, token1Detail?.decimals ?? 6)
              }
              : undefined,
  ...
  useEffect(() => {
    formUpdated?.([value1, token1, token2].some((item) => !item) ? false : true)
  }, [value1, token1, token2])


  const token1UnitPrice=tokenInfo.find((item)=>item?.token===token1)?.unitPrice
  ...
  const contents = [value1, token1, token2].some((item) => !item)
      ...
      : [
        {
          title: (
              <TooltipIcon content={Tooltip.Trade.DollarValue}>
                Expected Price
              </TooltipIcon>
          ),
          content: (
              bound((!isNaN(number(div(value1 ?? "0", value2 ?? 0))) && isFinite(div(value1 ?? "0", value2 ?? 0))) && <>{decimal(div(value1 ?? "0", value2 ?? 0), 6)}{' '}{symbol1 ?? ''}</>, <LoadingPlaceholder size={"sm"} className={styles.loading} color={"lightGrey"} />)
          ),
        },
        ...insertIf(token1!==USDC_addr && token2!==USDC_addr,
            {
              title: (
                  <TooltipIcon content={Tooltip.Trade.Price}>
                    Price per {getSymbol(token2)==='ujuno' ? 'JUNO' : getSymbol(token2) } in Dollars
                  </TooltipIcon>
              ),
              content: (
                  bound((!isNaN(number(div(value1 ?? "0", value2 ?? 0))) && isFinite(div(value1 ?? "0", value2 ?? 0))) && <>${decimal(multiple(div(value1 ?? "0", value2 ?? 0),token1UnitPrice), 6)}</>, <LoadingPlaceholder size={"sm"} className={styles.loading} color={"lightGrey"} />)
              ),
            }
        ...
        {
          title: (
              <TooltipIcon content={"The percentage change of the current price after you place this trade"}>Price Impact</TooltipIcon>
          ),
  ...
  const disabled = invalid || simulating || !!messages?.length || !pair || (simulating || loading ) || gt(priceImpact ?? '0','100')
  ...
  return (
      <CustomMsgFormContainer
          ...
          afterSubmitChilds={gt(priceImpact, "10") && (
              bound(<Button size={'lg'} className={styles.swapSubmit} type={'button'} onClick={toggleConfirmModal}>SWAP</Button>, "")
          )}
          showSubmitBtn={!(gt(priceImpact, "10"))}
          formSubmited={sumbit}
          isNewDesign={isNewDesign}
          mainSectionClass={isNewDesign ? styles.mainCard : ''}
      >
        {
            !makeCollapseable && (
                <>
                  {isNewDesign ? <FormGroupV2 {...fields[Key.value1]} /> : <FormGroup {...fields[Key.value1]} />}
        ...
        {isNewDesign ? <FormGroupV2 {...fields[Key.value2]} /> : <FormGroup {...fields[Key.value2]} />}
      </span>
                </>
            )
        }

        <Modal isOpen={isAnchorModalOpen} onClose={toggleConfirmModal} title={''} className={styles?.modalDesign}>
          {['uusd', aUST_TOKEN].includes(token1) && ['uusd', aUST_TOKEN].includes(token2) ?<UstAustHighPriceImpactModal />: <HighPriceImpactModal submitForm={() => setSumbit(true)} toggleConfirmModal={toggleConfirmModal} />}
        </Modal>
      </CustomMsgFormContainer>
  )
}
