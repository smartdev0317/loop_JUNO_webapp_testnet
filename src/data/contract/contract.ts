...
export const rawPairsQuery = selector({
  ...
  get: async ({ get }) => {
    ...
    const filter_pairs = [...pairs]?.filter((item) =>  !['terra1vdrxu6mskdgy0w0jcrtmjlrddfyulgwgj88dtw', 'terra1vduap6etw4663p0t9x534s95f6ghmg8nk2f4mh'].includes(item.contract_addr))
...
export const junoTokensList = selector({
  key: "junoTokensList",
  get: async ({ get }) => {
    let sortedContractsList: CONTRACT[] = [];
    poolList?.['juno'].map((item) =>{
      const assets = item.pool_assets
      const token1 = assets[0].native ? assets[0].denom : assets[0].token_address;
      const token2 = assets[1].native ? assets[1].denom : assets[1].token_address;
      if (token1) {
        sortedContractsList = sortedContractsList.filter((item) => item.token !== token1)
        sortedContractsList.push({
          contract_addr: token1,
          token: token1,
          denom: assets[0].native ? token1 : '',
          isNative: assets[0].native,
          pair: item.swap_address,
          lp: item.staking_address,
          tokenSymbol: '',
          tokenName: '',
          secondToken: token2 ?? "",
          decimals: number(assets[0].decimals) ?? 6,
        })
      }
      if (token2) {
        sortedContractsList = sortedContractsList.filter((item) => item.token !== token2)
        sortedContractsList.push({
          contract_addr: token2,
          token: token2,
          denom: assets[1].native ? token2 : '',
          isNative: assets[1].native,
          pair: item.swap_address,
          lp: item.staking_address,
          tokenSymbol: '',
          tokenName: '',
          secondToken: token1 ?? "",
          decimals: number(assets[1].decimals) ?? 6,
        })
      }
      return sortedContractsList;
    })
...
export const wyndTokensList = selector({
  key: "wyndTokensList",
  get: async ({ get }) => {
    let sortedContractsList: CONTRACT[] = [];
    poolList?.['wynd'].map((item) =>{
      const assets = item.pool_assets
      const token1 = assets[0].native ? assets[0].denom : assets[0].token_address;
      const token2 = assets[1].native ? assets[1].denom : assets[1].token_address;
      if (token1) {
