...
import { getFarmStakedJQuery, getFarmTotalStakedJQuery } from "./juno_queries"
...
export const listOfStakeableTokensQuery = selector({
  ...
  get: async ({ get }) => {
    ...
    return await client.queryContractSmart(pairContracts["loop_farm_staking"] ?? '', { query_list_of_stakeable_tokens: {
      limit:30
    } })
  },
})
...
export const useStakedByUserFarmQuery = () => {
  return useStoreLoadable(getFarmStakedJQuery, stakedByUserFarmQueryState)
}
...
export const useFarminglpTokenBalanceParsed = () => {
  return useStoreLoadable(getFarmTotalStakedJQuery, farminglpTokenBalanceParsedState)
}
