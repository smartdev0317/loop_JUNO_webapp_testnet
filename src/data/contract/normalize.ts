...
import {
  bankBalanceQuery, contractsList,
...
import {LOOP, LOOP_addr, SMALLEST, USDC_addr} from "../../constants"
...
import {getLpTokenBalanceJQuery, nativeDenom, useBalancesList, useJunoBalancesList} from "./juno_queries"
import { useJunoTokenBalance } from "../../hooks/newHooks/useTokenBalance"
import {tokensInfo, tokensInfoForLoop, tradingListStore, unitPricesStore} from "../API/dashboard"
...
export const useLpTokenBalances = () => {
  return useStoreLoadable(getLpTokenBalanceJQuery, lpTokenBalancesState)
}
...
export const usePairPool = () => {
  const { contents: factory1, isLoading} = usePairsPool()
...
export const usePairsPool = () => {
  return useStore(getPairPoolQuery, pairPoolState)
}
...
export const LoopPriceQuery = selector({
  key: "LoopPriceQuery",
  get: ({ get }) => {
    const data = get(tokensInfoForLoop)
    const loop=data?.find(item=>item?.token===LOOP_addr)
    return loop?.unitPrice;
  },
})
...
export const useFindBalance = () => {
  ...
  const{ contents: all_list } = useBalancesList()
  const junox = useJunoTokenBalance(nativeDenom)
  const dictionary = {
    // ...nativeBalances,
    // ...tokenBalances,
    ...lpTokenBalances,
