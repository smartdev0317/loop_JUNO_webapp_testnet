...
import { contractsList, getLoopPairsQuery } from "./contract"
    ...
    if (contracts) {
      ...
      await Promise.all<any>(
        contracts?.map(async (query) =>{
         const data = await client.queryContractSmart(query ?? '', { pool: {}});
         obj[query] = data
...
export const useGetTokenListV2 = () => {
  ...
  return (type: string, unique: boolean = true) => {
    ...
    contracts?.map((list: CONTRACT) => {
      ...
      unique ? !duplicat && uniqueList.push(list) : uniqueList.push(list)
    })
    return uniqueList;
  }
}
