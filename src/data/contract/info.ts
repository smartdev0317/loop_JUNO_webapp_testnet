...
import { dict, nativeBalancesQuery, useNativeBalances } from "./normalize"
...
import { tokensInfoStore } from "../API/common"
...
export const tokensInfoQuery = selector({
  ...
  get: async ({ get }) => {
   ...
   const result =  get(tokensInfoStore)
   const obj = {}
...
export const findTokensInfoQuery = selector({
  key: "findTokensInfoQuery",
  get: async ({ get }) => {
   const result =  get(tokensInfoQuery)
   return (token?: string) => token ? result[token] : {}
  },
})
