...
const devTokenBalanceQueryState = atomFamily<any,any>({
    key: "devTokenBalanceQueryState",
    default: { balance: "0"},
})


export const useDevTokenBalanceQuery = (devToken:string) => {
    return useStore(devTokenBalanceQuery({devToken}), devTokenBalanceQueryState(devToken))
}
...
export const queryGetUserAutoCompoundSubriptionFarmQuery = selector({
    key: "queryGetUserAutoCompoundSubriptionFarmQuery",
    get: async ({ get }) => {
                get(priceKeyIndexState)
                const { contracts } = get(protocolQuery)
...
const queryGetUserAutoCompoundSubriptionFarmState = atom<any>({
    key: "queryGetUserAutoCompoundSubriptionFarmState",
    default: {},
})

export const useGetUserAutoCompoundSubription = () => {
    return useStore(
        queryGetUserAutoCompoundSubriptionFarmQuery,
        queryGetUserAutoCompoundSubriptionFarmState
    )
}
