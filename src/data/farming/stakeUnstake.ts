...
import { getFarmMyRewardsJQuery } from "../contract/juno_queries";
...
export const getDevTokensByLp = selector({
    ...
    const { contracts: pairContracts } = get(protocolQuery)
    ...
    if (contracts) {
      ...
      await Promise.all<any>(
        contracts?.map(async (query) => {
          const data = await client.queryContractSmart(pairContracts["loop_farm_staking"] ?? "", { query_flp_token_from_pool_address: { pool_address: query } })
          obj[query] = data
...
export const useUserRewardInPool = () => {
    return useStoreLoadable(getFarmMyRewardsJQuery, queryUserRewardInPoolState)
}
...
export const getUserStakedTimeQuery = selector({
    ...
    get: async ({ get }) => {
        ...
        const data = await client.queryContractSmart(contracts['loop_farm_staking'] ?? "", { query_user_staked_time: { pool: query.lp, wallet: address } })
        obj[query.lp] = data
