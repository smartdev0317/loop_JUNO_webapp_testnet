...
import {
    ...
    getListedLpContractQueriesQuery, getListedLpContractQueriesV2Query,
} from "../utils/queries"
import {
    dict,
...
import { contractsList, contractsQuery } from "../contract/contract"
...
export const stakedByUserFarmQueryFarm2 = selectorFamily({
  ...
  get: (type: FarmContractTYpe) => async ({ get }) => {
      get(priceKeyIndexState)
      const { contracts } = get(protocolQuery)
      ...
      if (list) {
        ...
        // const getListedContractQueries = type === FarmContractTYpe.Farm4 ? get(getListedLpContractQueriesV2Query) : get(getListedLpContractQueriesQuery)
...
export const getLastDistributionNextPayoutQueryFarm2 = selectorFamily({
  ...
  get:
    ...
    if (contracts) {
      ...
      await Promise.all<any>(
        list?.map(async (query) => {
          const data = await client.queryContractSmart(contracts['loop_farm_staking'] ?? "", { query_last_distribution_time: { pool_address: query.lp } })
          obj[query.lp] = data
          return { balance: data, token: query.lp }
        })
      )
...
export const getLockTimeFrameQueryFarm2 = selectorFamily({
  ...
  get:
    ...
    async ({ get }) => {
      ...
      if(contracts[type]){
        return await client.queryContractSmart(contracts[type] ?? '', { query_lock_time_frame: { }});
    }else{
      return undefined
    }},
})

export const getLockTimeFrameForAutoCompoundQuery = selector({
    ...
    get:async ({ get }) => {
                ...
                if(contracts){
                  return await client.queryContractSmart(contracts['loop_farm_staking'] ?? '', { query_lock_time_frame_for_auto_compound: { }});
              }
              return "0"
            },
})
...
export const getTotalStakedForFarmingQuery = selectorFamily({
    ...
    if (contracts) {
      ...
      await Promise.all<any>(
        list?.map(async (query) => {
          const data = await client.queryContractSmart(contracts['loop_farm_staking'] ?? "", {
            query_total_staked: { staked_token: query.lp },
          })
          obj[query.lp] = data
          return { balance: data, token: query.lp }
        })
...
const getLastDistributionInPoolState = atom<any>({
  key: "getLastDistributionInPoolState",
  default: {},
})

export const useGetLastDistributionInPool = () => {
  return useStore(getLastDistributionInPool, getLastDistributionInPoolState)
}

export const getLastDistributionInPool = selector({
  key: "getLastDistributionInPool",
  get:async ({ get }) => {
      get(priceKeyIndexState)
      const {contracts} = get(protocolQuery)
      const list = get(stakedableTokens)
      const { client } = get(walletState)
    if (!client) {
      return
    }
    ...
    if (contracts) {
      let obj = {}
      await Promise.all<any>(
        list?.map(async (query: { liquidity_token: string, token: string}) => {
          const data = await client.queryContractSmart(contracts['loop_farm_staking'] ?? "", {
            query_last_distribution_time: {
              pool_address: query.token
            },
          })
          obj[query.token] = data
          return { balance: data?.balance, token: query.liquidity_token }
        })
      )
      return obj
    }
...
export const getLastDistributionInPoolIndex = selector({
  key: "getLastDistributionInPoolIndex",
  get: async ({ get }) => {
      get(priceKeyIndexState)
      const list: any = get(stakedableTokens)
      if(list){
      const getIndex = (lp: string) => {
          const index = list?.findIndex((item: any) => item.token === lp)
          return list[index - 1]?.token ?? undefined
      }
...
export const stakedableTokens = selector({
  key: "stakedableTokens",
  get: async ({ get }) => {
    const { contracts } = get(protocolQuery)
    const { client } = get(walletState)
    if (!client) {
