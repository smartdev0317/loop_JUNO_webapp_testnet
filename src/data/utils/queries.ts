...
import {contractsFactory2Query, contractsQuery, getLoopPairsQuery, rawPairsFactory2Query} from "../contract/contract"
import {
...
export const getListedLpContractQueriesV2Query = selector({
  key: "getListedLpContractQueriesV2Query",
  get: ({ get }) => {
    const contracts  = get(contractsFactory2Query)
    const getContractQueries = get(getContractQueriesQuery)
    return async <Parsed>(fn: GetTokenInfoDocument, name: string) => {
      const document = alias(
          contracts ? contracts
