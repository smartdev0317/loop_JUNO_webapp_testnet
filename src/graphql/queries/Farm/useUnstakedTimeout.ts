...
export const useUnstakeTimoutFarm2 = (stakedTime: string | undefined, type: FarmContractTYpe, lp: string | undefined) => {
  const getLockTimeFrame = useRecoilValue(getLockTimeFrameQueryFarm2(FarmContractTYpe.Farm1))
  const [timeString, setTimeString] = useState('')
  const [dayString, setDayString] = useState('')
  const [timeLeft, setTimeLeft] = useState(0)
  const [shortTimeString, setShortTimeString] = useState('')
...
export const useLockTimeFrameForAutoCompound = (stakedTime: string | undefined) => {
  const getLockTimeFrame = useRecoilValue(getLockTimeFrameForAutoCompoundQuery)
  const [timeString, setTimeString] = useState('')
  const [dayString, setDayString] = useState('')
  const [timeLeft, setTimeLeft] = useState(0)
  const [shortTimeString, setShortTimeString] = useState('')
  ...
  useEffect(()=>{
    ...
    days >0 && setShortMonthsString(`${lte(days, 9) ? `0${days}` : days}`)
    days > 0 ? setShortMonth(`${lte(days, 9) ? `${days}` : days}`) : setShortMonth('0')
    days > 0 && setShortDayString(`${lte(daysSm, 9) ? `${daysSm}` : daysSm}`)
    setShortTimeString(timeArray.map((item)=> `${lte(time[item], 9) ? `0${time[item]}` : `${time[item]}`}`).join(':'))
  },[stakedTime, currentTime])
