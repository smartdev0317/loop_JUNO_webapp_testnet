...
const ChartContainer = ({
  ...
  const data: any = (canvas: any) => {
    ...
    const data1Label:any=datasets?.map((price)=> moment(parseInt(price?.t))).flat();
    const data1Set:any=datasets?.map((price)=> (price?.y.toString()));
      ...
      xAxes: [
        {
          ...
          time: {
            ...
            unitStepSize: 1,
            displayFormats: {
              hour: (currentRangeTab == '1h' || currentRangeTab == '12h' || currentRangeTab == '1d') ? "H:m" : 'DD MMM',
            },
          },
          display: !compact,
          ticks: {
